    ...
    <article className={style.post}>
        ...
        <main className={style.main}>
            <Link to={`/post/${post.id}`} className={style.link}>
                ...
                {post.imageUrl !== "" && <img src={post.imageUrl} className={style.mainImage}/> }
            </Link>
        </main>
        ...
        <footer className={style.footer}>
            ...
            <div className={style.footerButtonsContainer}>
                <FontAwesomeIcon icon={faHeart} className={likeStatus === true ? "color--blue" : "hover--color-blue"} onClick={async()=> await onLike()}/>
                {post.likes}

                <Link to={`/post/${post.id}`}><FontAwesomeIcon icon={faComment} className="hover--color-blue"/></Link>
                {post.Comments.length}
                
            </div>

            <p className={style.date}>{new Date(post.createdAt).toLocaleString()}</p>
