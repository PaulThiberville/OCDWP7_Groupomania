      ...
      <nav className={style.nav}>
        <div className={style.links}>
          ...
          <Link to={"/profile/" + user.userId} className={style.link}>
            <FontAwesomeIcon icon={faUser} className={style.icon} />Profile
          </Link>
        </div>

        <div className={style.buttons}>
          <button onClick={()=> setIsPostOpen(true)} className="color--blue">Post</button>
          <Modal open={isPostOpen} onClose={()=> setIsPostOpen(false)}>
            <CreatePostForm onClose={()=> setIsPostOpen(false)} />
          </Modal>

          <button className="color--red" onClick={async (e)=> await onLogout(e)}>Logout</button>
        </div>
